...
if __name__ == '__main__':
    ...
    c1_bins = pandas.cut([-1, 1], bins=n_bins, retbins=True)[1][1:-1]
    #print(cart_position_bins)
    p1_bins = pandas.cut([-1, 1], bins=n_bins_angle, retbins=True)[1][1:-1]
    #print(pole_angle_bins)
    c2_bins = pandas.cut([-1, 1], bins=n_bins, retbins=True)[1][1:-1]
    #print(cart_velocity_bins)
    p2_bins = pandas.cut([-1, 1], bins=n_bins_angle, retbins=True)[1][1:-1]

    c3_bins = pandas.cut([-12.57 , 12.57], bins=n_bins, retbins=True)[1][1:-1]
    #print(cart_velocity_bins)

    p3_bins = pandas.cut([-23.8, 23.8], bins=n_bins_angle, retbins=True)[1][1:-1]
    ...
    print("Best 100 meaning score: {:0.2f}".format(reduce(lambda x, y: x + y, trajectory[0:100]) / len(trajectory[:100])))
